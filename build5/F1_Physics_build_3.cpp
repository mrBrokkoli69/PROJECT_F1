...
void F1PhysicsEngine::shiftUp() {
    if ((current_state.current_gear < 8) && (current_state.current_gear > 0)) {
        current_state.current_gear++;
        double gear_factor = params.gear_ratios[current_state.current_gear] * params.final_drive;
        current_state.engine_rpm = current_state.wheel_rpm * gear_factor;
    } else if (current_state.current_gear == 0) {
        current_state.current_gear++;
        double gear_factor = params.gear_ratios[current_state.current_gear] * params.final_drive;
        calculateTorque(); //нужно дополнить, чтобы реализовать сцепление на старте
        current_state.engine_rpm = current_state.wheel_rpm * gear_factor;
    }
}
...
void F1PhysicsEngine::shiftDown() {
    if (current_state.current_gear > 1) {
        double new_gear_factor = params.gear_ratios[current_state.current_gear - 1] * params.final_drive;
...
void F1PhysicsEngine::calculateRPM(bool gas_pedal, double dt) {
    calculateWheelLoads();
    double mean_wheel_load = (current_state.wheel_loads[0] + current_state.wheel_loads[1]
                            + current_state.wheel_loads[2] + current_state.wheel_loads[3]) / 4.0;
    ...
    if (current_state.current_gear != 0) {
        ...
        new_engine_rpm = current_state.wheel_rpm * gear_factor - params.rotor_resist * dt;

        if (gas_pedal) {
            new_engine_rpm += dt * params.acceleration_rate_max;
            current_state.engine_rpm = new_engine_rpm - dt * mean_wheel_load * pow(gear_factor, 2) * pow(params.wheel_radius, 2) / params.rotor_inertia
                                        / 9.81 * abs(current_state.engine_rpm - new_engine_rpm);
        } else {
            current_state.engine_rpm -= dt * mean_wheel_load * pow(gear_factor, 2) * pow(params.wheel_radius, 2) / params.rotor_inertia
                                        / 9.81 * abs(current_state.engine_rpm - new_engine_rpm);
        ...
        current_state.engine_rpm -= params.rotor_resist * dt;
        if (gas_pedal) {
            current_state.engine_rpm += dt * params.acceleration_rate_max;
        }
...
void F1PhysicsEngine::TestMaxRPMForCurrentGear() { //проверка максимальных оборотов для дальнейше связи с TractionForce
    if (current_state.current_gear > 0) {
        ...
        max_wheel_rpm = params.max_rpm / gear_factor;
...
void F1PhysicsEngine::calculateForces(bool gas_pedal, bool brake_pedal, double steering) {
    ...
    current_state.brake_force = calculateBrakeForce();
...
void F1PhysicsEngine::integrateMotion(double dt) {
    ...
    // 3. ОБНОВЛЯЕМ RPM КОЛЕС И ДВИГАТЕЛЯ НА ОСНОВЕ РЕАЛЬНОЙ СКОРОСТИ
     ...
     double target_wheel_rpm = current_state.speed / ((2 * M_PI / 60.0) * params.wheel_radius);
