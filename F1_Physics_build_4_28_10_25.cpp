...
void F1PhysicsEngine::calculateForces(bool gas_pedal, bool brake_pedal, double steering) {
    ...
    // ПРОСТОЙ КОНТРОЛЬ: если достигли максимума RPM - отключаем тягу
    double gear_ratio = params.gear_ratios[current_state.current_gear - 1];
    double gear_factor = gear_ratio * params.final_drive;
    double max_wheel_rpm = params.max_rpm / gear_factor;
    
    if (current_state.wheel_rpm >= max_wheel_rpm * 0.98 && gas_pedal) {
        current_state.traction_force = 0.0; // Отключаем тягу у предела
    } else {
...
void F1PhysicsEngine::integrateMotion(double dt) {
    ...
    // 2. ВЫЧИСЛЯЕМ СКОРОСТЬ ИЗ RPM КОЛЕС И ОГРАНИЧИВАЕМ КИНЕМАТИЧЕСКУЮ СКОРОСТЬ
    double gear_ratio = params.gear_ratios[current_state.current_gear - 1];
    double gear_factor = gear_ratio * params.final_drive;
    double max_wheel_rpm = params.max_rpm / gear_factor;
    double max_speed_from_rpm = max_wheel_rpm * (2 * M_PI / 60.0) * params.wheel_radius;
    ...
    // ЖЕСТКОЕ ОГРАНИЧЕНИЕ СКОРОСТИ ПО ОБОРОТАМ
    if (current_speed > max_speed_from_rpm) {
        double scale = max_speed_from_rpm / current_speed;
        current_state.velocity.x *= scale;
        current_state.velocity.y *= scale;
        current_speed = max_speed_from_rpm;
    }
    
    current_state.position.x += current_state.velocity.x * dt;
    current_state.position.y += current_state.velocity.y * dt;
    
    // 3. ОБНОВЛЯЕМ RPM КОЛЕС И ДВИГАТЕЛЯ НА ОСНОВЕ КИНЕМАТИЧЕСКОЙ СКОРОСТИ
    double target_wheel_rpm = current_speed / ((2 * M_PI / 60.0) * params.wheel_radius);
    
    // Плавная коррекция RPM колес
    current_state.wheel_rpm += (target_wheel_rpm - current_state.wheel_rpm) * 0.5 * dt;
    
    // ОБРАТНАЯ СВЯЗЬ: обновляем RPM двигателя из RPM колес
    current_state.engine_rpm = current_state.wheel_rpm * gear_factor;
    
    // 4. Обновление скорости и позиций колес
