...
int main() {
    ...
    noecho();
    keypad(stdscr, TRUE);
    timeout(33);  // Таймаут 33ms вместо nodelay
    curs_set(0);
    ...
    while (running) {
        ...
        mvprintw(34, 2, "W - Gas: %s", gas_pressed ? "PRESSED" : "RELEASED");
        mvprintw(35, 2, "S - Brake: %s", brake_pressed ? "PRESSED" : "RELEASED");
        ...
        mvprintw(40, 0, "RPM PROGRESS: %.1f%%", rpm_progress);
        int bar_width = 40;
        ...
        // === CSV ЛОГИРОВАНИЕ (каждые 15 кадров) ===
        frame_counter++;
        if (frame_counter >= 15) {
        ...
        switch (ch) {
            ...
            case ERR: // Таймаут - нет ввода
                // Ничего не делаем, просто продолжаем
                break;
        }
        
        // Если клавиши не нажаты в этом кадре, сбрасываем газ и тормоз
        if (ch != 'w' && ch != 'W' && ch != 's' && ch != 'S') {
            // Газ и тормоз сбрасываются только если не нажаты
            // Но для возможности "держать" кнопки - убираем автосброс
            // gas_pressed = false;
            // brake_pressed = false;
        }
        
        refresh();
        // timeout уже установлен, поэтому sleep не нужен
    }
